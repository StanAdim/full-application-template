...
const movePage = async (type:number) => {
  ...
  await userStore.retrieveUserList(per_page.value,currentPage.value)
}
// change page number
...
const  searchUserData = async () => {
  await userStore.retrieveUserList(per_page.value,currentPage.value, searchQuery.value)
}
const headers = ref(['Sn', 'Name', "Email" ,"Phone", 'Registration Date','Last Login','Verification', 'Active', 'Actions'])
const OpenConfirmDialog = async (uid) => {
  ...
  await globalData.deleteItemInDB( uid, 'user')
      .then(() => {
        ElMessage({
          type: 'success',
...
const handleAction = async  (type, uid) => {
  switch (type){
    case 1: {
      // navigateTo(`/admin/users/user/${uid}`)
      break;
    }
    case 2: {
      await adminStore.switchApprovalStatus('users',uid)
      break;
    }
    case 3: {
...
// initialize
const init = async () =>  {
  globalData.assignPageTitle('Registered Users')
  await userStore.retrieveUserList(per_page.value,currentPage.value, searchQuery.value)
}
onNuxtReady(()=> {
  init()
    ...
    <div class="w-full  py-2">
      ...
      <div class="overflow-auto rounded-lg shadow-lg">
        <table class="w-full bg-white rounded-lg overflow-x-auto">
          ...
          <tr v-if="userStore.getUserList?.length != 0"
              v-for="(item, index) in userStore.getUserList"
              :key="item.uid"
            ...
            <td class="table-data">{{ item?.lastLogin }}</td>
            <td class="table-data">
              <span class="mx-2 text-lg">
              <i v-if="item?.isVerified" class="fa-solid fa-circle-check text-green-500"></i>
              ...
              <el-dropdown size="default" type="primary" placement="bottom-start">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item @click.prevent="handleAction(2, item.uid)"  ><i class="fa-solid fa-hashtag"></i> Inactive</el-dropdown-item>
