...
const OpenConfirmDialog = async (uid) => {
  ...
  await globalData.deleteItemInDB( uid, 'hub')
      .then(() => {
        ElMessage({
          type: 'success',
...
const handleAction = async  (type, uid) => {
  switch (type){
    case 1: {
      navigateTo(`/admin/profiles/hubs/${uid}`)
      break;
    }
    case 2: {
...
// initialize
const init = async () =>  {
  globalData.assignPageTitle('Innovation Hub Profiles')
  await adminStore.retrieveProfileList('hubs', per_page.value,currentPage.value, searchQuery.value)
}
onNuxtReady(()=> {
  init()
    ...
    <div class="w-full  py-2">
      ...
      <div class="overflow-auto rounded-lg shadow-lg">
        <table class="w-full bg-white rounded-lg overflow-x-auto">
          ...
          <tr v-if="adminStore.getHubList?.length != 0"
              v-for="(item, index) in adminStore.getHubList"
              :key="item.uid"
