...
const init = async () => {
  globalData.assignPageTitle('Digital Accelerator Details')
  await  adminStore.retrieveSingleProfile('accelerators', route.params.uid)
}
onNuxtReady(() => {
  init()
})
...
<template>
  <div class="profile-view-container p-4 max-w-full mx-auto bg-white rounded-lg shadow-md">
    ...
    <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div>
        <!--        Unique Profile Info-->
        <AuthTheAcceleratorInfo :data="adminStore.getSingleProfile" />
