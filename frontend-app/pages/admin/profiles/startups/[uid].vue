...
const init = async () => {
  globalData.assignPageTitle('ICT Startup Details')
  await  adminStore.retrieveSingleProfile('startups', route.params.uid)
}
onNuxtReady(() => {
    init()
})
...
<template>
  <div class="profile-view-container p-4 max-w-full mx-auto bg-white rounded-lg shadow-md">
    ...
    <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div>
        <!--        Unique Profile Info-->
        <AuthTheStartupInfo :data="adminStore.getSingleProfile"  />
      </div>
      <div>
